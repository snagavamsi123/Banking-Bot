...
def ChatData(request):
    if request.method == 'GET':
        ...
        if question== ' ' or question=='':
            question = 'you have entered a space'
...
def UserLogin(request):
    if request.method == 'POST':
      ...
      index = 0
      con = pymysql.connect(host='127.0.0.1',user = 'root2',port = 3306, password = '1234', database = 'bankingbotapp',charset='utf8') #,port = 3308
      with con:    
          cur = con.cursor()
          print('cur values is ',cur)
          cur.execute("select * FROM bankingbotapp_register")
          rows = cur.fetchall()
          for row in rows: 
             if row[1] == username and password == row[2]:
                index = 1
                break		
      if index == 1:
...
def Signup(request):
    if request.method == 'POST':
      ...
      db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root2', password = '1234', database = 'bankingbotapp',charset='utf8')#
      db_cursor = db_connection.cursor()
      student_sql_query = "INSERT INTO bankingbotapp_register(username,password,contact,email,address) VALUES('"+username+"','"+password+"','"+contact+"','"+email+"','"+address+"')"
